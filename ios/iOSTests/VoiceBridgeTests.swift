...
final class VoiceBridgeTests: XCTestCase {
    ...
    func testDeliveryPartialFinalFailed() {
        ...
        XCTAssertEqual(VoiceBridge.delivery(for: state(.ready, id: id, partial: "x", final: "今天下午三點"), expecting: id), .final("今天下午三點"))
    ...
    func testEmptyFinalIsStillDelivered() {
        let id = UUID()
        XCTAssertEqual(VoiceBridge.delivery(for: state(.ready, id: id, final: ""), expecting: id), .final(""))
    }
